...
export function ReviewerDashboard() {
  ...
  const submitted = submittedQ.data ?? [];
  const underReview = reviewQ.data ?? [];
  ...
  return (
    <Bento>
      ...
      <StatTile index={1} eyebrow="AWAITING" dot={STATUS_META.SUBMITTED.c} value={submitted.length}
                label="To be claimed" to="/queue" />
      <StatTile index={2} eyebrow="UNDER REVIEW" dot={STATUS_META.UNDER_REVIEW.c} value={underReview.length}
                label="In progress" color={underReview.length ? tokens.review : undefined} to="/queue" />
      ...
      <ListTile
        ...
        empty="Queue is clear — nothing awaiting review."
      />

      <StatTile index={5} eyebrow="OLDEST WAIT" mono value={oldest}
                label="Longest in the queue" color={oldestUrgent ? tokens.rejected : undefined} to="/queue" />
      <StatTile index={6} eyebrow="FIRMS" value={firmsInQueue} label="With deals waiting" to="/queue" />

      <ActionTile
        index={7}
        actions={[
          { to: '/queue', label: 'Review queue', icon: <InboxIcon fontSize="small" />, primary: true },
